...
$strConsulta2 = "SELECT * 
                        from materia 
                        INNER JOIN curso on curso.id_curso = materia.id_curso
                        where materia.borrado is null";
...
$pdf->Cell(0, 6, "Listado de Materias", 0, 1);
...
for ($i = 0; $i < $numfilas; $i++) {
    ...
    if ($i % 2 == 1) {
        ...
        $pdf->SetTextColor(0);
        $pdf->Row(array($fila['nombre'],$fila['curso']));
    } else {
        $pdf->SetFillColor(186, 186, 186);
        $pdf->SetTextColor(0);
        $pdf->Row(array($fila['nombre'],$fila['curso']));
